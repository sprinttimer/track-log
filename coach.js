...
function buildCoachComment(current,history=[],profile={},nextMeet=null){
  ...
  return parts.slice(0,5).join(" ") || "記録を保存すると、PB比・反復低下・RPE・前後疲労を組み合わせてコメントします。";
}
...
function periodMetrics(trainings,baseISO){
  ...
  const recentHigh=recentRun.filter(x=>["high","very_high"].includes(intensityFromRatio(x.pbRatio))).length;
  const baseWeeklyHigh=baseRun.filter(x=>["high","very_high"].includes(intensityFromRatio(x.pbRatio))).length/4;
...
function buildWeeklyAnalysis(trainings,profile={},nextMeet=null,baseISO=isoOf(new Date())){
  ...
  if(!m.recent.length) return{text:"直近7日間の記録がありません。練習前後の疲労感、RPE、標準ランニングのPB比を入力すると傾向を比較できます。",metrics:m};
  if(m.baselineReady){
    ...
    if(m.recentHigh>=m.baseWeeklyHigh+1.2&&m.recentHigh>=2) parts.push(pick([`高強度走が${m.recentHigh}回あり、通常より密度が高くなっています。`,`PB比93%以上の高強度走が${m.recentHigh}回で、普段より高強度日の間隔が詰まっています。`],seed+4));
    else if(m.recentHigh===0&&m.baseWeeklyHigh>=1) parts.push("普段は入っている高強度走が直近7日にはありません。調整週・回復週であれば自然な変化です。");
  ...
  if(nextMeet){ const gap=daysBetween(baseISO,nextMeet.date); if(gap>=0&&gap<=3) parts.push(`次の大会まで${gap}日です。ここからは新しい負荷を足すより、開始前疲労が下がるかを優先して確認してください。`); else if(gap>=4&&gap<=7&&m.recentHigh>=2) parts.push(`大会まで${gap}日で、高強度走が直近7日に${m.recentHigh}回あります。試合日に疲労を残さない間隔になっているか確認してください。`); }
...
function buildRaceAnalysis(meet,trainings,profile={}){
  ...
  const pre7=trainings.filter(x=>inWindow(x,d7start,beforeEnd)), pre3=trainings.filter(x=>inWindow(x,d3start,beforeEnd)), baseline=trainings.filter(x=>inWindow(x,baseStart,baseEnd));
  const run7=pre7.filter(isRunningRecord), run3=pre3.filter(isRunningRecord), baseRun=baseline.filter(isRunningRecord);
  ...
  const high7=run7.filter(x=>["high","very_high"].includes(intensityFromRatio(x.pbRatio))).length, high3=run3.filter(x=>["high","very_high"].includes(intensityFromRatio(x.pbRatio))).length;
  const fatigueAvg=mean(pre7.map(x=>fatiguePair(x).before).filter(x=>x>0)), baseFat=mean(baseline.map(x=>fatiguePair(x).before).filter(x=>x>0));
  const uniqueDays=new Set(pre7.map(x=>x.date)).size, restDays=Math.max(0,7-uniqueDays);
  const highs=[...run7].filter(x=>["high","very_high"].includes(intensityFromRatio(x.pbRatio))).sort((a,b)=>b.date.localeCompare(a.date));
  const lastHighGap=highs[0]?daysBetween(highs[0].date,raceDate):null;
  const seed=hashSeed({meet,pre7:pre7.length}), parts=[];
  if(volumeRatio!=null){
  ...
  if(high3>=2) parts.push(pick([`試合3日前以内に高強度走が${high3}回あり、直前まで強い刺激が続いています。`,`試合前3日間にPB比93%以上の走が${high3}回あります。疲労が残っていなかったか確認してください。`],seed+2));
  else if(high3===1) parts.push(`試合3日前以内に高強度走が1回あります。最後の刺激として意図したものか、当日の疲労感と合わせて振り返ってください。`);
  else if(high7>0&&lastHighGap!=null) parts.push(`最後の高強度走は試合${lastHighGap}日前でした。直前3日には高強度走が入っていません。`);
  else parts.push("試合前7日にはPB比93%以上の標準ランニング記録がありませんでした。");
  ...
  return{text:parts.slice(0,6).join(" "),metrics:{volumeRatio,high7,high3,fatigueAvg,restDays,lastHighGap,pre7Count:pre7.length}};
}
