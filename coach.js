...
function sessionItems(x){
  ...
  return (x.items||[]).map(item=>({
    ...
    date:x.date,
    rpe:Number(x.rpe||item.rpe||0),
    fatigueBefore:Number(x.fatigueBefore||item.fatigueBefore||item.fatigue||0),
    fatigueAfter:Number(x.fatigueAfter||item.fatigueAfter||item.fatigue||0),
    durationMin:Number(x.durationMin||item.durationMin)||null,
    notes:x.notes||item.notes||"",
    _sessionParentId:x.id||null
  }));
}
...
function sessionVolumeComment(current,history=[],seed=0){
  ...
  const flat=expandTrainings(history);
  const same=flat.filter(isRunningRecord).filter(x=>x.category===current.category&&runningVolume(x)>0&&x.date<=current.date).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,10);
...
function conditionComments(x,seed=0){
  ...
  if(before>=4) out.push(pick([`開始前の疲労感が${before}/5と高めです。直近の練習量と回復状況を確認してください。`,`練習前から疲労感${before}/5です。今日の数値だけでなく、数日間の推移も合わせて確認してください。`,`開始時点の疲労感が高い状態です（${before}/5）。高強度を連続させていないか履歴を見直す価値があります。`],seed));
  if(after===5) out.push(pick(["練習後の疲労感が5/5です。次の高強度日まで回復を優先してください。","終了後の疲労感が最大値です。翌日の開始前疲労が下がるかを確認してください。"],seed+1));
  else if(delta>=2&&after>=4) out.push(pick([`疲労感が${before}/5→${after}/5へ大きく上昇しました。今日は十分な刺激が入った可能性があります。`,`前後の疲労差が+${delta}です。次回の練習前に回復しているかを確認してください。`],seed+2));
...
function buildCoachComment(current,history=[],profile={},nextMeet=null){
  ...
  const ratio=current.pbRatio??pbRatio(current.pb,current.averageTime), drop=current.dropPct??maxDrop(current.times||[]);
  const parts=[], hasTime=Number.isFinite(Number(current.averageTime))&&Number(current.averageTime)>0;
  if(!hasTime) parts.push(pick(["タイムは未計測です。速度・PB比による評価は行わず、距離・本数・セット・前後の疲労感から内容を判定します。","今回はタイムなしの記録です。未計測をマイナス評価には使わず、練習量と主観的負荷、前後の疲労感を中心に見ています。","タイムが入力されていないため速度評価だけを省略しています。距離・本数・セットとコンディション情報から通常どおり記録・分析します。"],seed));
  ...
  const categoryPhrases={
    ...
    speed_endurance:["ロングスプリントでは、終盤のタイム低下とフォーム維持をセットで評価すると進歩が見えやすくなります。","200〜400mなどのロングスプリントは負荷が高くなりやすいため、翌日の開始前疲労も確認してください。"],
  ...
  return parts.slice(0,5).join(" ") || "記録を保存すると、距離・本数・セット・前後疲労を基礎に、タイムがある場合のみPB比や反復低下も加えてコメントします。";
}
function buildSessionCoachComment(session,history=[],profile={},nextMeet=null){
  ...
  if(items.length>1){
    const labels=items.map(x=>isWeightRecord(x)?x.weightExerciseName||"ウェイト":isCustomRecord(x)?x.customMenuName||"カスタム":CATEGORY_LABELS[x.category]||x.category).filter(Boolean);
    parts.push(pick([`本日は${items.length}メニューを1つの練習日としてまとめて評価しています（${labels.slice(0,3).join("・")}${labels.length>3?"ほか":""}）。`,`今日の${items.length}メニューを個別ではなく、1日の練習全体として判定しています。`],seed));
  }
  ...
  if(nextMeet){ const gap=daysBetween(session.date,nextMeet.date); if(gap>=0&&gap<=3&&highItems.length) parts.push(`次の大会まで${gap}日です。今日の高負荷メニューを追加で重ねるより、疲労が下がるかを優先して確認してください。`); }
  if(weights.length||customs.length){
    const extraLabels=[]; if(weights.length)extraLabels.push(`ウェイト${weights.length}件`); if(customs.length)extraLabels.push(`カスタム${customs.length}件`);
    parts.push(`${extraLabels.join("・")}は内容自体を標準ランニング負荷へ換算せず、前後の疲労感を1日の総合状態に反映しています。`);
  }
  ...
  return parts.filter(Boolean).slice(0,6).join(" ") || "この日の複数メニューをまとめて保存し、走行量・タイム・前後疲労から1日単位で分析します。";
}
...
function buildRaceAnalysis(meet,trainings,profile={}){
  ...
  if(high3>=2) parts.push(pick([`試合3日前以内に高負荷の練習日が${high3}回あり、直前まで強い刺激が続いています。`,`試合前3日間に、タイムがある場合のPB比またはメニュー特性と疲労反応からみた高負荷日が${high3}回あります。疲労が残っていなかったか確認してください。`],seed+2));
  else if(high3===1) parts.push("試合3日前以内に高負荷の練習日が1回あります。最後の刺激として意図したものか、当日の疲労感と合わせて振り返ってください。");
  ...
  return{text:parts.slice(0,6).join(" "),metrics:{volumeRatio,high7,high3,fatigueAvg,restDays,lastHighGap,racePrev7Count:racePrev7.length}};
}
